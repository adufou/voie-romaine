...
func get_upgrade_cost(upgrade_id: String) -> int:
    if not upgrade_id in upgrade_definitions:
        log_message(["upgrade", "purchase"], "Amélioration inexistante: %s" % upgrade_id, "WARNING")
        return 0
...
func purchase_upgrade(upgrade_id: String) -> bool:
    if not is_started:
        log_message(["upgrade", "purchase"], "Tentative d'acheter une amélioration avant le démarrage complet du service", "WARNING")
        return false
    ...
    if cost < 0:
        log_message(["upgrade", "purchase"], "Niveau maximum atteint pour %s" % upgrade_id, "INFO")
        return false
        
    if not cash_service:
        log_message(["upgrade", "purchase"], "Cash service non configuré, impossible d'acheter une amélioration", "ERROR")
        return false
        
    if not cash_service.use_cash(cost):
        log_message(["upgrade", "purchase"], "Cash insuffisant: %d requis" % cost, "INFO")
        return false
        
    upgrades[upgrade_id] += 1
    
    log_message(["upgrade", "purchase"], "Amélioration %s achetée (niveau %d) pour %d or" % 
...
func get_upgrade_effect(upgrade_id: String) -> float:
    if not upgrade_id in upgrade_definitions:
        log_message(["upgrade", "info"], "Amélioration inexistante: %s" % upgrade_id, "WARNING")
        return 0.0
